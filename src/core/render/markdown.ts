...
// 공개 렌더는 renderHeaders()의 sandbox CSP(고유 오리진·connect-src none) 아래에서 서빙되므로,
// HTML 업로드 경로와 동일한 위협 모델이 적용된다. 즉 마크다운 원본에 섞인 임의 HTML/스크립트도
// 격리된 sandbox 안에서만 실행되며, 부모 오리진(쿠키/스토리지/네트워크)에는 접근할 수 없다.
// 따라서 HTML 업로드와 일관되게 별도 sanitize 없이 marked 출력을 그대로 문서로 감싼다.
marked.setOptions({ gfm: true, breaks: false });

// 미리보기(web/src/lib/markdown.ts)와 공개 렌더가 같은 결과를 내도록 CSS는 양쪽이 동일하게 유지한다.
const MARKDOWN_CSS = `
...
/** 마크다운 원본을 스타일이 인라인된 완결형 HTML 문서로 렌더한다(공개 /<path> 렌더용). */
